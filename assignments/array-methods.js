...
// Problem 1
// Donations over 50 are automatically entered for a prize contest
let donationsOver50 = runners.filter(runner => {
    return runner.donation > 50;
})
console.log(donationsOver50)

// Problem 2
// Get a total shirt list of sizes
let neededShirtSizeList = runners.map(runner => {
    return runner.shirt_size;
}).sort()

console.log(neededShirtSizeList);

// Problem 3
// company representative runner
let businessRunner = runners.map(runner => {
    return `Full name: ${runner.first_name} ${runner.last_name} Company: ${runner.company_name}`
})
console.log(businessRunner);
